...
bool model::build(const string & name)
{
    ...
    {//build/copy model
        ...
        for(list<polygon>::iterator i=data.polys.begin();i!=data.polys.end();i++)
		{
			...
			{
                tris[tid].v[vid]=*j;
				//copy texture coordinates. Note: it's better to keep another list of texture coords 
				//instead of copying textcoords to each triangle....but...
				//if (t != i->textures.end()) 
				//{
				//	tris[tid].texcoord[vid] = Vector2d(&data.textcoords[*t].x); //texture coordinates
				//	t++;
				//}
...
void model::transform(const Vector3d& T, const Matrix3x3& M, double S)
{
	Vector3d tmp;
